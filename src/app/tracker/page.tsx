  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        {/* Countdown Card */}
        <Card className="border-border shadow-sm flex flex-col justify-between bg-card/65 backdrop-blur-sm">
          ...
          <CardContent className="p-4 pt-1 sm:pt-2">
            {nextPeriodDate && !isCurrentlyInPeriod && (
              <p className="text-xs text-muted-foreground font-semibold">
                Kỳ tiếp theo dự kiến: <span className="text-foreground">{format(nextPeriodDate, "dd MMMM, yyyy", { locale: vi })}</span>
              </p>
            )}
            {isCurrentlyInPeriod && latestCycle && (
              <p className="text-xs text-muted-foreground font-semibold">
                Bắt đầu từ: <span className="text-foreground">{format(parseISO(latestCycle.startDate), "dd MMMM, yyyy", { locale: vi })}</span>
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 border-border shadow-sm overflow-hidden bg-card/75 backdrop-blur-sm">
          <CardHeader className="p-4 pb-3 sm:p-6 sm:pb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border/40">
            <div>
              ...
              <CardDescription className="text-[10px] sm:text-xs mt-1.5 flex flex-wrap gap-x-3 gap-y-1">
                <span className="inline-flex items-center gap-1">
                  <span className="w-2.5 h-2.5 rounded-full bg-primary inline-block"></span> Ngày hành kinh
                </span>
                <span className="inline-flex items-center gap-1">
                  <span className="w-2.5 h-2.5 rounded-full bg-secondary border border-dashed border-primary inline-block"></span> Ngày dự đoán
                </span>
              </CardDescription>
            </div>
            ...
            <div className="w-full sm:w-auto">
              {!isCurrentlyInPeriod ? (
                <Button 
                  ...
                  className="w-full sm:w-auto h-10 px-4 font-bold text-xs rounded-xl bg-primary text-primary-foreground hover:bg-primary/95 flex items-center justify-center gap-1.5 shadow-sm active:scale-98 transition-transform"
                >
                ...
                <Button 
                  ...
                  className="w-full sm:w-auto h-10 px-4 font-bold text-xs rounded-xl bg-accent text-accent-foreground hover:bg-accent/95 flex items-center justify-center gap-1.5 shadow-sm active:scale-98 transition-transform"
                >
          ...
          <CardContent className="flex flex-col md:flex-row items-center justify-center gap-6 p-4 sm:p-6">
            {/* Calendar Container */}
            <div className="p-2 sm:p-3 border border-border/80 bg-muted/20 rounded-2xl w-full max-w-[320px] md:max-w-[340px] flex justify-center overflow-hidden shadow-inner shrink-0">
              <Calendar
                mode="single"
                locale={vi}
            ...
            <div className="flex-1 w-full max-w-sm space-y-4 self-stretch flex flex-col justify-center">
              ...
              {showAddForm && (
                <form onSubmit={handleAddSubmit} className="bg-muted/40 p-4 border border-border rounded-2xl space-y-4 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-200">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      className="h-10 rounded-xl bg-background border-border text-sm font-semibold"
                      required
                    />
                  </div>
                  <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2 pt-1.5">
                    ...
                    <Button 
                      type="submit" 
                      className="flex-1 h-9 text-xs rounded-xl font-bold bg-primary text-primary-foreground"
                      disabled={loading}
                    >
                      Ghi nhận
                    </Button>
              ...
              {showEndForm && (
                <form onSubmit={handleEndSubmit} className="bg-muted/40 p-4 border border-border rounded-2xl space-y-4 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-200">
                  ...
                  <div className="flex gap-2 pt-1.5">
                    ...
                    <Button 
                      type="submit" 
                      className="flex-1 h-9 text-xs rounded-xl font-bold bg-accent text-accent-foreground"
                      disabled={loading}
                    >
                      Hoàn thành
                    </Button>
              ...
              {!showAddForm && !showEndForm && (
                <div className="p-4 bg-muted/15 border border-dashed border-border rounded-2xl text-center space-y-2.5 py-6">
        ...
        <Card className="border-border shadow-sm bg-card/75 backdrop-blur-sm flex flex-col">
          ...
          <CardContent className="p-4 flex-1 max-h-[420px] overflow-y-auto">
            {sortedCycles.length === 0 ? (
              <div className="py-12 text-center text-muted-foreground text-xs font-semibold">
                Chưa ghi nhận dữ liệu chu kỳ nào.
              </div>
            ) : (
              <div className="relative border-l-2 border-border pl-4 space-y-6 pt-2 ml-2">
                    ...
                    <div key={cycle.id} className="relative">
                      ...
                      <span className={`absolute -left-[23px] top-1.5 w-3 h-3 rounded-full border-2 ${
                        cycle.endDate 
                          ...
                          : "bg-green-500 border-background animate-pulse"
                      }`}></span>
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <button
                            ...
                            className="p-1 text-muted-foreground hover:text-red-500 hover:bg-muted rounded-lg transition-colors shrink-0"
                            title="Xóa chu kỳ"
                          >
                        ...
                        <div className="flex flex-wrap gap-1.5 text-[10px]">
                          {cycle.duration && (
                            <span className="px-2 py-0.5 rounded-md bg-secondary/40 text-secondary-foreground font-bold border border-secondary/20">
                              Hành kinh: {cycle.duration} ngày
                            </span>
                          )}
                          {cycle.cycleLength && (
                            <span className="px-2 py-0.5 rounded-md bg-muted text-muted-foreground font-bold border border-border/50">
                        ...
                        {cycle.isAbnormal && (
                          <div className="flex items-start gap-1.5 p-2.5 bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-300 border border-red-100 dark:border-red-950/40 rounded-xl text-[10px] leading-relaxed font-bold shadow-inner">
                        ...
                        {cycle.notes && (
                          <p className="text-[10px] sm:text-[11px] italic text-muted-foreground leading-normal bg-muted/20 p-2 rounded-xl border border-border/20 font-medium">
                            Ghi chú: {cycle.notes}
                          </p>
                        )}
                      </div>
