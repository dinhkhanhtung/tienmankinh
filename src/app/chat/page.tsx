...
export default function ChatPage() {
  ...
  const handleScroll = () => {
    ...
    // Nếu khoảng cách đến đáy lớn hơn 200px thì hiện nút
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-9.5rem)] md:h-[calc(100vh-7rem)] max-w-3xl mx-auto space-y-4 page-transition">
      {/* Header */}
      <div className="shrink-0">
        <h1 className="text-xl sm:text-2xl font-black tracking-tight text-foreground flex items-center gap-2">
          <MessageCircle className="w-5 h-5 sm:w-6 h-6 text-primary animate-pulse" /> Trò chuyện cùng AI Coach
        </h1>
      ...
      <Card className="flex-1 flex flex-col border-border/80 shadow-md overflow-hidden bg-card/85 backdrop-blur-md rounded-3xl min-h-0 glass-card">
        {/* Chat window Header */}
        <CardHeader className="bg-muted/5 border-b border-border/40 py-3 px-4 flex flex-row items-center justify-between shrink-0">
          ...
          >
                ...
                >
                  ...
                  <div
                    className={`max-w-[82%] rounded-2xl px-4 py-3 text-xs sm:text-sm leading-relaxed font-semibold shadow-sm transition-all duration-200 ${
                      isUser
                        ? "bg-gradient-to-br from-primary to-[#B05581] text-primary-foreground rounded-tr-none shadow-md shadow-primary/10"
                        : "bg-[#FFF5F8] dark:bg-[#20131B] text-[#3D2232] dark:text-[#FFF0F6] rounded-tl-none border border-[#F5E2EC]/70 dark:border-[#3D2232] hover:shadow-md"
                    }`}
                  >
            ...
            {sending && (
              <div className="flex w-full gap-2.5 items-start justify-start animate-in fade-in duration-200">
                ...
                <div className="bg-[#FFF5F8] dark:bg-[#20131B] rounded-2xl rounded-tl-none px-4 py-3 border border-[#F5E2EC]/70 dark:border-[#3D2232] flex items-center gap-1.5 shadow-sm min-w-[64px] h-[38px] justify-center">
        ...
        <CardFooter className="flex flex-col border-t border-border/40 p-3 sm:p-4 gap-3 bg-muted/5 shrink-0">
          ...
          {messages.length <= 2 && !sending && (
            <div className="w-full">
              <div className="flex items-center gap-1.5 text-[9px] text-primary/90 font-extrabold uppercase tracking-wider mb-2 select-none">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-none snap-x snap-mandatory">
                {quickQuestions.map((q, idx) => (
                  <button
                    ...
                    className="text-left text-[11px] text-primary font-bold bg-primary/5 hover:bg-primary/10 border border-primary/15 px-4 py-2 rounded-full transition-all shrink-0 snap-start max-w-[260px] truncate shadow-sm active:scale-95 duration-150"
          ...
          <div className="w-full flex items-start gap-2 text-[9px] leading-relaxed text-muted-foreground bg-amber-500/5 dark:bg-amber-500/10 p-2.5 rounded-xl border border-amber-500/10">
          ...
          {/* Chat Form Input */}
          <form onSubmit={handleFormSubmit} className="flex w-full items-center gap-2 pt-1.5">
            <Input
              placeholder="Nhập câu hỏi của chị..."
              value={input}
              onChange={(e) => setInput(e.target.value)}
              className="flex-1 h-12 rounded-full border-border bg-background focus-visible:ring-primary text-sm font-semibold pl-5 pr-3 shadow-inner placeholder:text-muted-foreground/60 focus:border-primary/55"
              disabled={sending}
              maxLength={500}
            />
            <Button
              type="submit"
              size="icon"
              className="h-12 w-12 rounded-full bg-primary text-primary-foreground hover:bg-primary/95 shadow-md shadow-primary/15 active:scale-90 transition-all shrink-0 cursor-pointer"
